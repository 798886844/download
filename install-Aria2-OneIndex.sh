...
basic_dependency(){
    apt update
    apt install wget unzip net-tools bc curl sudo screen -y     
}
...
filebrowser_install(){
echo -e "${Green} 开始安装FileBrowser ${Font}"
cd /root
mkdir filebrowser
cd filebrowser
wget -O linux-amd64-filebrowser.tar.gz https://github.com/798886844/download/raw/master/linux-amd64-filebrowser.tar.gz
tar -zxvf linux-amd64-filebrowser.tar.gz
screen -dmS filebrowser ./filebrowser --port 82 --address 0.0.0.0 --root /root/Download
cd /root
}
...
echo -e "
# ====================================================
#   Aria2+Aria2Ng+OneIndex+FileBrowser一键安装脚本
#   系统:Debian 8、9
#   运行环境:Nginx + php7
#   作者：chyon 修改自：Eleven
#   博客：https://chyon.cn
#
#   ${Red}Aria2Ng前端地址:${Green}http://${domain_ip}:81${Font}
#   ${Red}OneIndex地址:  ${Green}http://${domain_ip}:80${Font}
#   ${Red}FileBrowser地址:  ${Green}http://${domain_ip}:82${Font}
# ====================================================
"
}
